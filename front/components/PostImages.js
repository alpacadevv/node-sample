...
const PostImages = ({ images }) => {
  ...
  if (images.length === 2) {
    return (
      <>
        <div>
          <img src={`http://localhost:3065/${images[0].src}`} width="50%" alt={images[0].src} onClick={onZoom} />
          <img src={`http://localhost:3065/${images[1].src}`} width="50%" alt={images[1].src} onClick={onZoom} />
        </div>
        {showImagesZoom && <ImagesZoom images={images} onClose={onClose} />}
      </>
    );
